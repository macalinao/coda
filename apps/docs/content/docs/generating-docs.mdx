...
You can customize the documentation generation through your `coda.config.mjs` file:
...
When working with multiple programs, Coda generates separate documentation files for each:

```javascript
export default defineConfig({
  // Glob pattern for multiple IDLs
  idlPath: "./idls/*.json",
  
  docs: {
    // Same NPM package for all programs
    npmPackageName: "@my-org/my-programs",
  },
});
```
...
The `coda docs` command accepts the following options:

```bash
# Specify custom IDL path
coda docs -i ./path/to/idl.json
