...
Client generation can be customized through `coda.config.ts`. See the [Configuration](/docs/configuration) page for detailed options including:
...
For a real-world example with multiple programs, see the [Quarry configuration](https://github.com/macalinao/coda/blob/master/clients/quarry/coda.config.ts).
...
1. **Version Control**: Commit your `coda.config.ts` but consider gitignoring generated files
