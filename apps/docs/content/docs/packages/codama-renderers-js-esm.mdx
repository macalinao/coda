...
5. **Erasable Syntax**: Emits no TypeScript syntax that carries runtime behaviour
...
### 5. Erasable Syntax Only

`erasableSyntaxOnly` — and Node.js type stripping, which enforces the same rule — reject TypeScript syntax that carries runtime behaviour. The standard renderer emits `enum` declarations for IDL enums; this renderer emits a `const` object paired with a union type of the same name instead:
...
The object keeps the exact runtime shape a numeric `enum` compiles to, reverse mapping included, because `@solana/codecs` derives an enum's keys and values by inspecting that shape. `Version` remains usable as both a type and a value, so `Version.V1`, `getVersionEncoder()` and `getVersionDecoder()` are unchanged.

The program plugin's angle-bracket type assertion — also non-erasable, since it is ambiguous with JSX — becomes an equivalent `as` assertion.
...
| Enums             | `enum` declarations  | Erasable `const` objects           |
