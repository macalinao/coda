...
Options:
  ...
  -c, --config <path>   Path to config file (default: "./coda.config.ts")
```
...
Options:
  -c, --config <path>   Path for config file (default: "./coda.config.ts")
```
...
Create a `coda.config.ts` to customize generation:
