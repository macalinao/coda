---
title: Configuration
description: Customize Coda generation with coda.config.ts
---

Coda works out of the box with sensible defaults, but you can customize its behavior with a `coda.config.ts` file.
...
This creates a `coda.config.ts` file with helpful comments and examples.
...
See the [Quarry configuration](https://github.com/macalinao/coda/blob/master/clients/quarry/coda.config.ts) for a real-world example where instructions like `claimRewards`, `initMiner`, and `withdrawTokens` were renamed to avoid conflicts between the `quarryMine` and `quarryMergeMine` programs.
...
2. Configuration file (`coda.config.ts`)
3. Default values
...
- Ensure file is named `coda.config.ts` with `.ts` extension
