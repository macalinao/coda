...
/** Gets the encoder for {@link ConfigTransactionArgs} account data. */
...
/** Gets the decoder for {@link ConfigTransaction} account data. */
...
/** Gets the codec for {@link ConfigTransaction} account data. */
...
export async function fetchConfigTransactionFromSeeds(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  seeds: TransactionSeeds,
  config: FetchAccountConfig & { programAddress?: Address } = {},
): Promise<Account<ConfigTransaction>> {
  const maybeAccount = await fetchMaybeConfigTransactionFromSeeds(
    rpc,
    seeds,
    config,
  );
...
export async function fetchMaybeConfigTransactionFromSeeds(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  seeds: TransactionSeeds,
  config: FetchAccountConfig & { programAddress?: Address } = {},
): Promise<MaybeAccount<ConfigTransaction>> {
  ...
  return await fetchMaybeConfigTransaction(rpc, address, fetchConfig);
}
