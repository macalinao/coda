...
/** Gets the encoder for {@link VaultBatchTransactionArgs} account data. */
...
/** Gets the decoder for {@link VaultBatchTransaction} account data. */
...
/** Gets the codec for {@link VaultBatchTransaction} account data. */
...
export async function fetchVaultBatchTransactionFromSeeds(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  seeds: BatchTransactionSeeds,
  config: FetchAccountConfig & { programAddress?: Address } = {},
): Promise<Account<VaultBatchTransaction>> {
  const maybeAccount = await fetchMaybeVaultBatchTransactionFromSeeds(
    rpc,
    seeds,
    config,
  );
...
export async function fetchMaybeVaultBatchTransactionFromSeeds(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  seeds: BatchTransactionSeeds,
  config: FetchAccountConfig & { programAddress?: Address } = {},
): Promise<MaybeAccount<VaultBatchTransaction>> {
  const { programAddress, ...fetchConfig } = config;
  const [address] = await findBatchTransactionPda(seeds, { programAddress });
  return await fetchMaybeVaultBatchTransaction(rpc, address, fetchConfig);
}
